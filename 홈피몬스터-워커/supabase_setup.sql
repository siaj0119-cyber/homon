...
-- [보안 강화된 RLS 정책]
-- 모든 기존 정책 동적 제거 (중복 정책 에러 방지)
DO $$
DECLARE
    r RECORD;
BEGIN
    FOR r IN (SELECT policyname, tablename FROM pg_policies WHERE schemaname = 'public' AND tablename IN ('external_settings', 'leads', 'page_views')) LOOP
        EXECUTE format('DROP POLICY IF EXISTS %I ON public.%I', r.policyname, r.tablename);
    END LOOP;
END $$;

-- 1. external_settings: 누구나 조회(SELECT) 가능. 관리자만 수정 가능.
...
-- 2. leads: 누구나 등록(INSERT) 가능. 조회/수정/삭제는 관리자만 가능.
...
-- 3. page_views: 누구나 등록(INSERT) 및 조회(SELECT) 가능. 수정/삭제는 관리자만 가능.
